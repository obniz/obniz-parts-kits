class AIRobotKit {
  constructor() {
    ...
    this.proximity = this._proximityNameFromDistance(
      this._lastProximityChangeDistancde
    );
  ...
  wired(obniz) {
    ...
    this.distanceSensor = obniz.wired('GP2Y0A21YK0F', {
      vcc: 7,
      gnd: 8,
      signal: 9,
    });
    ...
    this.distanceSensor.start((distance) => {
      ...
      if (
        ...
        Math.abs(distance - this._lastProximityChangeDistancde) >=
        this.proximityHysterisis
      ) {
  ...
  async _loadCascadeModel() {
    ...
    const url =
      'https://unpkg.com/obniz-parts-kits@0.1.2/airobot/opencv3.4/haarcascade_frontalface_default.xml';
